    ...
    if KARMA and ply:EV_HasPrivilege("Karma") then
        ...
        if #players > 0 then
            evolve:Notify(evolve.colors.blue, ply:Nick(),
                evolve.colors.white, " changed ",
                evolve.colors.red, evolve:CreatePlayerList(players),
                evolve.colors.white, "'s karma to ",
                evolve.colors.red, tostring(value),
                evolve.colors.white, ".")
        else
...
function PLUGIN:Menu(arg, players)
    if arg then
        ...
        return "Set Karma", evolve.category.actions, {
            { "500", 500 },
            { "600", 600 },
            { "700", 700 },
            { "800", 800 },
